...
This document describes the target architecture for first-class remote environments in NitroCode.
...
- Keep the NitroCode server as the execution boundary.
...
NitroCode already has a clean runtime boundary: the client talks to a NitroCode server over HTTP/WebSocket, and the server owns orchestration, providers, terminals, git, and filesystem operations.
...
└───────────────┬──────────────────────────────┘
                │
                │ connects to one NitroCode server
                │
┌───────────────▼──────────────────────────────┐
│ Execution environment = one NitroCode server       │
│                                              │
...
The important decision is that remoteness is expressed at the environment connection layer, not by splitting the NitroCode runtime itself.

## Domain model

### ExecutionEnvironment

An `ExecutionEnvironment` is one running NitroCode server instance.
...
How does the client speak WebSocket to a NitroCode server?
...
- best fit for hosted or self-managed remote NitroCode deployments
...
For NitroCode, tunnels are best modeled as another `AccessEndpoint`, not as a different kind of environment.
...
- reach a machine
- probe it
- launch or reuse a remote NitroCode server
...
How does a NitroCode server come to exist on the target machine?
...
The user or operator already runs NitroCode on the target machine, and the client connects through a direct or tunneled WebSocket endpoint.
...
What should be different in NitroCode:
...
The recommended NitroCode flow is:

1. Desktop connects over SSH.
2. Desktop probes the remote machine and verifies NitroCode availability.
3. Desktop launches or reuses a remote NitroCode server.
...
This is the inverse of remote launch: a local NitroCode server is already running, and the client publishes it through a tunnel.
...
- A manually hosted NitroCode server might be reached through direct `wss`.
...
NitroCode already supports a WebSocket auth token on the server. That should become a first-class part of environment access rather than remaining an incidental query parameter convention.
...
NitroCode should not copy that part.

NitroCode already has the right runtime boundary:

- one NitroCode server per environment
- ordinary HTTP/WebSocket between client and environment

So NitroCode should borrow Zed's launch discipline, not its transport protocol.
