...
Methods mirror the `NativeApi` interface defined in `@nitrocode/contracts`:

- `providers.startSession`, `providers.sendTurn`, `providers.interruptTurn`
- `providers.respondToRequest`, `providers.stopSession`
- `shell.openInEditor`, `server.getConfig`

Codex, Claude Agent, OpenCode, and Cursor are wired as built-in provider kinds. Runtime availability still depends on each provider's local CLI/auth probe and server settings.
