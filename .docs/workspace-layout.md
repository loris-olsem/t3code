...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `nitrocode` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@nitrocode/shared/git`, `@nitrocode/shared/DrainableWorker`) — no barrel index.
