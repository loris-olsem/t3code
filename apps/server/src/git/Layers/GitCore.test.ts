...
const ServerConfigLayer = ServerConfig.layerTest(process.cwd(), {
  prefix: "nitrocode-git-core-test-",
});
  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nitrocode/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nitrocode/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "nitrocode/tmp-working" });

        const renamed = yield* (yield* GitCore).renameBranch({
          cwd: tmp,
          oldBranch: "nitrocode/tmp-working",
          newBranch: "nitrocode/feat/session",
        });

        expect(renamed.branch).toBe("nitrocode/feat/session-1");
        const branches = yield* (yield* GitCore).listBranches({ cwd: tmp });
        expect(branches.branches.some((branch) => branch.name === "nitrocode/feat/session")).toBe(
          true,
        );
        expect(branches.branches.some((branch) => branch.name === "nitrocode/feat/session-1")).toBe(
          true,
        );
        const current = branches.branches.find((branch) => branch.current);
        expect(current?.name).toBe("nitrocode/feat/session-1");
      }),
    );
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nitrocode/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nitrocode/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "nitrocode/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "nitrocode/tmp-working" });
        ...
        expect(renamed.branch).toBe("nitrocode/feat/session-2");
      }),
    );
    ...
    it.effect(
      ...
      () =>
        Effect.gen(function* () {
          ...
          yield* git(tmp, ["remote", "add", "origin", "git@github.com:pingdotgg/nitrocode.git"]);

          const remoteName = yield* core.ensureRemote({
            cwd: tmp,
            preferredName: "origin",
            url: "git@github.com:pingdotgg/nitrocode.git/",
          });
