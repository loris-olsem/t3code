...
const BaseTestLayer = makeProjectionPipelinePrefixedTestLayer(
  "nitrocode-projection-pipeline-test-",
);
...
it.layer(Layer.fresh(makeProjectionPipelinePrefixedTestLayer("nitrocode-base-")))(
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("nitrocode-projection-attachments-safe-")),
...
it.layer(
  Layer.fresh(
    makeProjectionPipelinePrefixedTestLayer("nitrocode-projection-attachments-overwrite-"),
  ),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(
    makeProjectionPipelinePrefixedTestLayer("nitrocode-projection-attachments-rollback-"),
  ),
)("OrchestrationProjectionPipeline", (it) => {
    ...
    Effect.gen(function* () {
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointRef: CheckpointRef.make(
            "refs/nitrocode/checkpoints/thread-revert-files/turn/1",
          ),
          status: "ready",
          files: [],
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointRef: CheckpointRef.make(
            "refs/nitrocode/checkpoints/thread-revert-files/turn/2",
          ),
          status: "ready",
          files: [],
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("nitrocode-projection-attachments-revert-")),
)("OrchestrationProjectionPipeline", (it) => {
...
it.layer(
  Layer.fresh(makeProjectionPipelinePrefixedTestLayer("nitrocode-projection-attachments-delete-")),
)("OrchestrationProjectionPipeline", (it) => {
      ...
      Effect.gen(function* () {
        ...
        yield* appendAndProject({
          ...
          payload: {
            ...
            turnId: TurnId.make("turn-completed"),
            checkpointTurnCount: 1,
            checkpointRef: CheckpointRef.make("refs/nitrocode/checkpoints/thread-conflict/turn/1"),
            status: "ready",
            files: [],
    ...
    Effect.gen(function* () {
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointTurnCount: 1,
          checkpointRef: CheckpointRef.make("refs/nitrocode/checkpoints/thread-revert/turn/1"),
          status: "ready",
          files: [],
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointTurnCount: 2,
          checkpointRef: CheckpointRef.make("refs/nitrocode/checkpoints/thread-revert/turn/2"),
          status: "ready",
          files: [],
    ...
    Effect.provide(
      Layer.provideMerge(
        ServerConfig.layerTest(process.cwd(), {
          prefix: "nitrocode-projection-pipeline-restart-",
        }),
        NodeServices.layer,
      ),
    ),
...
const engineLayer = it.layer(
  OrchestrationEngineLive.pipe(
    ...
    Layer.provideMerge(
      ServerConfig.layerTest(process.cwd(), {
        prefix: "nitrocode-projection-pipeline-engine-dispatch-",
      }),
    ),
    Layer.provideMerge(NodeServices.layer),
  ),
