...
const APP_USER_MODEL_ID = isDevelopment ? "com.nitrocode.nitrocode.dev" : "com.nitrocode.nitrocode";
const LINUX_DESKTOP_ENTRY_NAME = isDevelopment ? "nitrocode-dev.desktop" : "nitrocode.desktop";
const LINUX_WM_CLASS = isDevelopment ? "nitrocode-dev" : "nitrocode";
const USER_DATA_DIR_NAME = isDevelopment ? "nitrocode-dev" : "nitrocode";
...
function backendChildEnv(): NodeJS.ProcessEnv {
  ...
  delete env.NITROCODE_PORT;
  delete env.NITROCODE_MODE;
  delete env.NITROCODE_NO_BROWSER;
  delete env.NITROCODE_HOST;
  delete env.NITROCODE_DESKTOP_WS_URL;
  delete env.NITROCODE_DESKTOP_LAN_ACCESS;
  delete env.NITROCODE_DESKTOP_LAN_HOST;
  return env;
}
...
function resolveAdvertisedHostOverride(): string | undefined {
  const override = process.env.NITROCODE_DESKTOP_LAN_HOST?.trim();
...
function resolveEmbeddedCommitHash(): string | null {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as { nitrocodeCommitHash?: unknown };
    return normalizeCommitHash(parsed.nitrocodeCommitHash);
  } catch {
    return null;
  }
}
...
function resolveAboutCommitHash(): string | null {
  ...
  const envCommitHash = normalizeCommitHash(process.env.NITROCODE_COMMIT_HASH);
  if (envCommitHash) {
...
function handleFatalStartupError(stage: string, error: unknown): void {
  ...
  if (!isQuitting) {
    isQuitting = true;
    dialog.showErrorBox("NitroCode failed to start", `Stage: ${stage}\n${message}${detail}`);
  }
  stopBackend();
  restoreStdIoCapture?.();
  app.quit();
...
function handleCheckForUpdatesMenuClick(): void {
  const hasUpdateFeedConfig =
    readAppUpdateYml() !== null || Boolean(process.env.NITROCODE_DESKTOP_MOCK_UPDATES);
  const disabledReason = getAutoUpdateDisabledReason({
    isDevelopment,
    isPackaged: app.isPackaged,
    platform: process.platform,
    appImage: process.env.APPIMAGE,
    disabledByEnv: process.env.NITROCODE_DISABLE_AUTO_UPDATE === "1",
    hasUpdateFeedConfig,
  });
  if (disabledReason) {
...
async function checkForUpdatesFromMenu(): Promise<void> {
  ...
  if (updateState.status === "up-to-date") {
    void dialog.showMessageBox({
      ...
      message: `NitroCode ${updateState.currentVersion} is currently the newest version available.`,
      buttons: ["OK"],
    });
 ...
 * parentheses (e.g. `~/.config/NitroCode (Alpha)` on Linux). This is
 * unfriendly for shell usage and violates Linux naming conventions.
 *
 * We override it to a clean lowercase name (`nitrocode`). If the legacy
...
function configureAutoUpdater(): void {
  const githubToken =
    process.env.NITROCODE_DESKTOP_UPDATE_GITHUB_TOKEN?.trim() || process.env.GH_TOKEN?.trim() || "";
  if (githubToken) {
  ...
  if (process.env.NITROCODE_DESKTOP_MOCK_UPDATES) {
    autoUpdater.setFeedURL({
      provider: "generic",
      url: `http://localhost:${process.env.NITROCODE_DESKTOP_MOCK_UPDATE_SERVER_PORT ?? 3000}`,
    });
  }
...
async function bootstrap(): Promise<void> {
  ...
  const configuredBackendPort = resolveConfiguredDesktopBackendPort(process.env.NITROCODE_PORT);
  if (isDevelopment && configuredBackendPort === undefined) {
    throw new Error("NITROCODE_PORT is required in desktop development.");
  }
