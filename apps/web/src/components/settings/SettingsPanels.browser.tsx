...
describe("GeneralSettingsPanel observability", () => {
  ...
  it("shows diagnostics inside About with a single logs-folder action", async () => {
    ...
    await expect
      .element(page.getByText("/repo/project/.nitrocode/logs", { exact: true }))
      .toBeInTheDocument();
    await expect
      .element(
        page.getByText(
  ...
  it("shows a disabled network access toggle with guidance in desktop builds", async () => {
    ...
    await expect
      .element(
        page.getByText("NitroCode will restart to expose this environment over the network."),
      )
      .toBeInTheDocument();
  ...
  it("opens the logs folder in the preferred editor", async () => {
    ...
    expect(openInEditor).toHaveBeenCalledWith("/repo/project/.nitrocode/logs", "cursor");
  });
