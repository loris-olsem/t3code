import { EnvironmentId, type GitBranch } from "@nitrocode/contracts";
...
describe("resolveDraftEnvModeAfterBranchChange", () => {
  it("switches to local mode when returning from an existing worktree to the main worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        ...
        currentWorktreePath: "/repo/.nitrocode/worktrees/feature-a",
  ...
  it("uses worktree mode when selecting a branch already attached to a worktree", () => {
    expect(
      resolveDraftEnvModeAfterBranchChange({
        nextWorktreePath: "/repo/.nitrocode/worktrees/feature-a",
        currentWorktreePath: null,
...
describe("resolveEffectiveEnvMode", () => {
  it("treats draft threads already attached to a worktree as current-checkout mode", () => {
    expect(
      resolveEffectiveEnvMode({
        activeWorktreePath: "/repo/.nitrocode/worktrees/feature-a",
...
describe("resolveCurrentWorkspaceLabel", () => {
  ...
  it("describes the active checkout as a worktree when one is attached", () => {
    expect(resolveCurrentWorkspaceLabel("/repo/.nitrocode/worktrees/feature-a")).toBe(
      "Current worktree",
    );
  });
});
...
describe("resolveLockedWorkspaceLabel", () => {
  ...
  it("uses a shorter label for an attached worktree", () => {
    expect(resolveLockedWorkspaceLabel("/repo/.nitrocode/worktrees/feature-a")).toBe("Worktree");
  });
});
...
describe("resolveBranchSelectionTarget", () => {
  it("reuses an existing secondary worktree for the selected branch", () => {
    expect(
      resolveBranchSelectionTarget({
        ...
        branch: {
          isDefault: false,
          worktreePath: "/repo/.nitrocode/worktrees/feature-b",
        },
      }),
    ).toEqual({
      checkoutCwd: "/repo/.nitrocode/worktrees/feature-b",
      nextWorktreePath: "/repo/.nitrocode/worktrees/feature-b",
  ...
  it("keeps checkout in the current worktree for non-default branches", () => {
    expect(
      ...
      checkoutCwd: "/repo/.nitrocode/worktrees/feature-a",
      nextWorktreePath: "/repo/.nitrocode/worktrees/feature-a",
      reuseExistingWorktree: false,
    });
  });
