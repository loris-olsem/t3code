import { EnvironmentId, ProjectId, ThreadId } from "@nitrocode/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.nitrocode/worktrees/nitrocode-mvp/nitrocode-4e609bb8",
    );
    expect(result).toBe("nitrocode-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.nitrocode\\worktrees\\nitrocode-mvp\\nitrocode-4e609bb8",
    );
    expect(result).toBe("nitrocode-4e609bb8");
  });

  it("uses the final segment even when outside ~/.nitrocode/worktrees", () => {
