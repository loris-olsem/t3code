...
- Publishes the CLI package (`apps/server`, npm package `nitrocode`) with OIDC trusted publishing from the same workflow file:
...
- Publishes the CLI package (`apps/server`, npm package `nitrocode`) to the `nightly` npm dist-tag using the same nightly version.
...
- Repository slug source:
  - `NITROCODE_DESKTOP_UPDATE_REPOSITORY` (format `owner/repo`), if set.
  - otherwise `GITHUB_REPOSITORY` from GitHub Actions.
- Temporary private-repo auth workaround:
  - set `NITROCODE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
...
1. Confirm npm org/user owns package `nitrocode` (or rename package first if needed).
