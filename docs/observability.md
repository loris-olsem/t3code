...
NitroCode has one server-side observability model:
...
Completed spans are written as NDJSON records to `serverTracePath` (by default, `~/.nitrocode/userdata/logs/server.trace.ndjson`).
...
```bash
export NITROCODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces
export NITROCODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics
export NITROCODE_OTLP_SERVICE_NAME=nitrocode-local
```

Optional:

```bash
export NITROCODE_TRACE_MIN_LEVEL=Info
export NITROCODE_TRACE_TIMING_ENABLED=true
```
...
Launch the actual app executable from the same shell so the desktop app and embedded backend inherit `NITROCODE_OTLP_*`.

macOS app bundle example:

```bash
NITROCODE_OTLP_TRACES_URL=http://localhost:4318/v1/traces \
NITROCODE_OTLP_METRICS_URL=http://localhost:4318/v1/metrics \
NITROCODE_OTLP_SERVICE_NAME=nitrocode-desktop \
"/Applications/NitroCode.app/Contents/MacOS/NitroCode"
```
...
```bash
tail -f "$NITROCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -c 'select(.exit._tag != "Success") | {
  ...
  exit,
  attributes
}' "$NITROCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
jq -r 'select(.traceId == "TRACE_ID_HERE") | [
  ...
  (.parentSpanId // "-"),
  .durationMs
] | @tsv' "$NITROCODE_HOME/userdata/logs/server.trace.ndjson"
```
...
- service name such as `nitrocode-local`, `nitrocode-dev`, or `nitrocode-desktop`
...
- `nitrocode_rpc_request_duration`
- `nitrocode_orchestration_command_duration`
- `nitrocode_orchestration_command_ack_duration`
- `nitrocode_provider_turn_duration`
- `nitrocode_git_command_duration`
- `nitrocode_db_query_duration`

Counters tell you volume and failure rate:

- `nitrocode_rpc_requests_total`
- `nitrocode_orchestration_commands_total`
- `nitrocode_provider_turns_total`
- `nitrocode_git_commands_total`
- `nitrocode_db_queries_total`
...
`nitrocode_orchestration_command_ack_duration` measures:
...
1. Check `nitrocode_orchestration_command_ack_duration` by `commandType`.
...
- `NITROCODE_OTLP_TRACES_URL` was not set
...
- `NITROCODE_TRACE_FILE`: override trace file path
- `NITROCODE_TRACE_MAX_BYTES`: per-file rotation size, default `10485760`
- `NITROCODE_TRACE_MAX_FILES`: rotated file count, default `10`
- `NITROCODE_TRACE_BATCH_WINDOW_MS`: flush window, default `200`
- `NITROCODE_TRACE_MIN_LEVEL`: minimum trace level, default `Info`
- `NITROCODE_TRACE_TIMING_ENABLED`: enable timing metadata, default `true`

OTLP export:

- `NITROCODE_OTLP_TRACES_URL`: OTLP trace endpoint
- `NITROCODE_OTLP_METRICS_URL`: OTLP metric endpoint
- `NITROCODE_OTLP_EXPORT_INTERVAL_MS`: export interval, default `10000`
- `NITROCODE_OTLP_SERVICE_NAME`: service name, default `nitrocode-server`
