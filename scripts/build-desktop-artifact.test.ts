...
it.layer(NodeServices.layer)("build-desktop-artifact", (it) => {
  ...
  it("switches desktop packaging product names to nightly for nightly builds", () => {
    assert.equal(resolveDesktopProductName("0.0.17"), "NitroCode (Alpha)");
    assert.equal(resolveDesktopProductName("0.0.17-nightly.20260413.42"), "NitroCode (Nightly)");
  });
        ...
        Effect.provide(
          ConfigProvider.layer(
            ConfigProvider.fromEnv({
              env: {
                NITROCODE_DESKTOP_SKIP_BUILD: "true",
                NITROCODE_DESKTOP_KEEP_STAGE: "true",
                NITROCODE_DESKTOP_SIGNED: "true",
                NITROCODE_DESKTOP_VERBOSE: "true",
                NITROCODE_DESKTOP_MOCK_UPDATES: "true",
              },
            }),
          ),
        ),
